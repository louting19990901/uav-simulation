...
void UAVNode::finish()
{
    ...
    if (utilizationFail && missionId >= 0)
...
void UAVNode::selectNextCommand()
{
    ...
    {
        std::string error_msg = "selectNextCommand(): " + std::string(this->getFullName()) + " should have switched into Exchange CEE now. Another UAV is waiting...";
        //        throw cRuntimeError(error_msg.c_str());
    ...
    bool atReplacementLocation = (abs(replacementX - x) + abs(replacementY - y) + abs(replacementZ - z)) < ERROR_MARGIN && abs((replacementTime - simTime()).dbl()) < 10 * ERROR_MARGIN;
    ...
    else
    {
        ...
        {
            std::string error_msg = "selectNextCommand(): replacingNode for " + std::string(this->getFullName()) + " should be known by now (part of hack111). Battery critical (" + std::to_string(battery.getRemainingPercentage()) + "%)?";
            throw cRuntimeError(error_msg.c_str());
...
void UAVNode::collectStatistics()
{
    if (commandExecEngine == nullptr)
        throw cRuntimeError("collectStatistics(): Command Engine missing.");

    // Only collect if executed CEE is active
    if (not commandExecEngine->isActive())
        return;
...
{
    if (commandExecEngine == nullptr)
        throw cRuntimeError("updateState(): Command Engine missing.");
...
{
    if (commandExecEngine == nullptr)
        throw cRuntimeError("isCommandCompleted(): Command Engine missing.");
...
{
    if (commandExecEngine == nullptr)
        throw cRuntimeError("nextNeededUpdate(): Command Engine missing.");
...
void UAVNode::loadCommands(CommandQueue commands, bool isMission)
{
    ...
    {
        EV_WARN << __func__ << "()"
                << " Replacing non-empty CEE queue." << endl;
        cees.clear();
    }

    for (u_int index = 0; index < commands.size(); ++index)
    {
        EV_INFO << __func__ << "index is " << index << endl;
...
float UAVNode::energyToNearestCN(double fromX, double fromY, double fromZ)
{
    ...
    if (nullptr == cn)
        throw omnetpp::cRuntimeError("No charging station available!");
