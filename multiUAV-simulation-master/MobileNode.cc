...
void MobileNode::handleMessage(cMessage *msg)
{
    ...
    else
    {
        ...
        if (commandPreview && getEnvir()->isGUI())
            drawCommandPreview();
    }
...
void MobileNode::drawCommandPreview()
{
    if (not waypoints.empty())
        waypoints.clear();
    if (not waypointsMaint.empty())
        waypointsMaint.clear();
    ...
    {
        if (commandPreviewCommandCount != 0 && countDrawnCommands == commandPreviewCommandCount)
            break;
